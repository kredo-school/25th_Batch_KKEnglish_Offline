            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div class="btn-group" role="group" aria-label="week navigation">
                    <a class="btn btn-outline-secondary"
                       href="{{ route('admin.schedules.index', ['week_start' => $prev]) }}">
                        ← Prev
                    </a>
                    <a class="btn btn-outline-primary"
                       href="{{ route('admin.schedules.index', ['week_start' => $curr]) }}">
                        This Week
                    </a>
                    <a class="btn btn-outline-secondary"
                       href="{{ route('admin.schedules.index', ['week_start' => $next]) }}">
                        Next →
                    </a>
                </div>
                        ...
                        <table class="table table-bordered text-center align-middle mb-0">
                            ...
                            <tr>
                                ...
                                @foreach(($dashboardRows ?? []) as $row)
                                    <td>
                                        @if(($row['capacity'] ?? 0) > 0)
                                            <a href="{{ route('admin.schedules.index_details', ['date' => $row['date'], 'type' => 'capacity']) }}">
                                                {{ $row['capacity'] }}
                                            </a>
                                        @else
                                            0
                            ...
                            <tr>
                                ...
                                @foreach(($dashboardRows ?? []) as $row)
                                    <td>
                                        @if(($row['booked'] ?? 0) > 0)
                                            <a href="{{ route('admin.schedules.index_details', ['date' => $row['date'], 'type' => 'booked']) }}">
                                                {{ $row['booked'] }}
                                            </a>
                                        @else
                                            0
                                ...
                                @foreach(($dashboardRows ?? []) as $row)
                                    <td>
                                        @if(($row['auto_booked'] ?? 0) > 0)
                                            <a href="{{ route('admin.schedules.index_details', ['date' => $row['date'], 'type' => 'auto_booked']) }}">
                                                {{ $row['auto_booked'] }}
                                            </a>
                                        @else
                                            0
                                ...
                                @foreach(($dashboardRows ?? []) as $row)
                                    <td>
                                        @if(($row['working_teachers'] ?? 0) > 0)
                                            <a href="{{ route('admin.schedules.index_details', ['date' => $row['date'], 'type' => 'working_teachers']) }}">
